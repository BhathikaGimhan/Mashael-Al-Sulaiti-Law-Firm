...
@section('title', 'Mashael Alsulaiti Law Firm | Edit Article ')
@section('content')
    ...
    <section class="card border-0 py-1 p-md-2 p-xl-3 p-xxl-4 mb-4">
        <div class="card-body">
            <form action="{{ route('admin.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="col-md-12 mb-3">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('article_title') is-invalid @enderror"
                                id="article_title" name="article_title" value="{{ $article->article_title }}"
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select id="lang_type" class="form-control @error('lang_type') is-invalid @enderror"
                                ...
                                <option value="en"
                                    {{ old('lang_type', $article->lang_type) === 'en' ? 'selected' : '' }}>
                                    {{ __('English') }}</option>
                                <option value="fr"
                                    {{ old('lang_type', $article->lang_type) === 'fr' ? 'selected' : '' }}>
                                    {{ __('French') }}</option>
                                <option value="ar"
                                    {{ old('lang_type', $article->lang_type) === 'ar' ? 'selected' : '' }}>
                                    {{ __('Arabic') }}</option>
                            </select>
                            @error('lang_type')
                                <span class="invalid-feedback" role="alert">
                ...
                <div class="my-3">
                    <div id="holder" class="my-3" style="max-height:120px;">
                        <img src="{{ $article->featured_image_public_path }}" alt="{{ $article->article_title }}"
                            style="height: 100px; width: auto;">
                    </div>
                    <div class="form-group">
                        <label class="fw-bold opacity-75 mb-1" for="featured_image_public_path">Featured Image:</label>
                        <div class="input-group mt-2">
                            ...
                            <input id="thumbnail"
                                ...
                                value="{{ $article->featured_image_public_path }}">
                ...
                <div class="form-group mb-4">
                    ...
                    <textarea id="article_description" class="form-control @error('article_description') is-invalid @enderror"
                        name="article_description" rows="5">{{ old('article_description', $article->article_description) }}</textarea>
                ...
                <div class="form-group">
                    <label class="fw-bold opacity-75 mb-1" for="article_content">Article Content</label>
                    <textarea class="form-control @error('article_content') is-invalid @enderror" id="article_content"
                        name="article_content" placeholder="Enter article_content">{{ old('article_content') }}{{ $article->article_content }}</textarea>
